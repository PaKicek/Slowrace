...
## Операторы
Операторы преобразуют одно или два значения переменных в их результат. Обе переменные должны быть совместимых типов данных, иначе вызывается ошибка `Type Error`.
...
- `==` - сравнение двух значений (любого типа).
...
```
Output: `Переменная a больше переменной b`
...
Output: `Содержание строк разное`

## Циклы
Есть 2 вида циклов: `for` и `while`.
